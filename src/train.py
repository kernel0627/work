...
def main() -> None:
    ...
    epochs_no_improve = 0
    if args.resume:
        ckpt = load_checkpoint(args.resume, model=model, optimizer=optimizer, scaler=scaler, map_location=device)
        start_epoch = int(ckpt.get('epoch', 0)) + 1
        best_ap = float(ckpt.get('best_metric', -1.0))
        extra = ckpt.get('extra', {})
        epochs_no_improve = int(extra.get('epochs_no_improve', 0))
        logger.info(f'resumed from {args.resume}, start_epoch={start_epoch}, best_ap={best_ap:.6f}')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint(ckpts_dir / 'last.pt', model, optimizer, scaler, epoch, best_ap, extra=extra)
        if (epoch + 1) % args.save_every == 0:
            save_checkpoint(ckpts_dir / f'epoch_{epoch}.pt', model, optimizer, scaler, epoch, best_ap, extra=extra)

        if val_metrics['ap'] > best_ap:
            best_ap = float(val_metrics['ap'])
            epochs_no_improve = 0
            extra['epochs_no_improve'] = epochs_no_improve
            save_checkpoint(ckpts_dir / 'best.pt', model, optimizer, scaler, epoch, best_ap, extra=extra)
